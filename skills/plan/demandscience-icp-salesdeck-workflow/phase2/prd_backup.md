...
- **Feature 3: Branded PowerPoint Generation**
  ...
  - Include Slides 3-4 as-is from template (static GTM Fabric content - no modifications)
  - Generate one slide per ICP (Slides 5+)
  - Acceptance: Output matches template design exactly, all placeholders replaced, static slides preserved
...
# Stage 2: PowerPoint Generation (custom skill + pptx skill)
container={
    "skills": [
        {"type": "custom", "skill_id": "demandscience-icp-skill-id", "version": "latest"},
...
Based on Claude Skills benchmarking:
- **Stage 1** (ICP Research & Markdown): 2-3 minutes
- **Stage 2** (PowerPoint Generation): 1-2 minutes
- **Total workflow**: Under 10 minutes ✅
- **Token efficiency**: ~90% reduction vs. manual prompting with full template specifications
...
├── REFERENCE.md          # Optional: Template documentation
