...
### Required API Patterns (Claude Skills Beta)

All API calls must use the **beta namespace** with proper configuration:
...
# ❌ WRONG: Standard namespace without beta configuration
response = client.messages.create(...)  # Will not work with Skills
```

### Stage 1: ICP Generation API Call

**Purpose**: Generate research-validated ICPs in Markdown format
...
stage1_response = client.beta.messages.create(
    ...
    container={
        "skills": [
            {
                "type": "custom",
                "skill_id": DEMANDSCIENCE_SKILL_ID,  # Your uploaded custom skill
                "version": "latest"
            }
        ]
    },
...
# Extract Markdown ICPs from response
markdown_icps = ""
...
print(f"Stage 1 Tokens: {stage1_response.usage.input_tokens} in, {stage1_response.usage.output_tokens} out")

# Show Markdown to user for review and approval
print(markdown_icps)
```

**Expected Output**: Plain text Markdown with 3+ ICPs
...
try:
    ...
    if "skill not found" in str(e).lower():
        print("❌ Skill not uploaded. Run: upload_skill.py")
    else:
        raise e
```
...
approval = input("Approve for PowerPoint generation? (yes/no): ")

if approval.lower() != "yes":
    print("⚠️ User did not approve. Workflow stopped.")
    sys.exit(0)

# Proceed to Stage 2 only after approval
stage2_response = client.beta.messages.create(...)
```
...
total_input_tokens = (
    stage1_response.usage.input_tokens +
    stage2_response.usage.input_tokens
)
total_output_tokens = (
...
print(f"""
Token Usage Summary:
  Stage 1: {stage1_response.usage.input_tokens} in, {stage1_response.usage.output_tokens} out
  Stage 2: {stage2_response.usage.input_tokens} in, {stage2_response.usage.output_tokens} out
  Total:   {total_input_tokens} in, {total_output_tokens} out

Expected: ~5,000-8,000 total tokens (90% reduction vs. manual prompting)
""")
```
